...
export default function Chat() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [chatRes, projectRes] = await Promise.all([
  ...
  const handleSend = async (e) => {
    ...
    try {
      const res = await api.post(`/chat/${projectId}`, { message: userMessage });
  ...
  return (
    <div className="relative flex flex-col h-[calc(100vh-56px)] bg-dark overflow-hidden">
      <div
      ...
      <header className="relative z-10 shrink-0 bg-dark-100/60 backdrop-blur-glass-lg px-5 py-3.5 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="font-semibold text-sm text-white truncate">{projectName || 'Chat'}</h2>
        ...
        {messages.length > 0 && (
          <button
            onClick={handleClear}
            className="text-xs text-dark-700 hover:text-red-400 transition-colors px-3 py-1.5 rounded-lg hover:bg-white/[0.03]"
          >
            Clear
          </button>
        )}
